...
<div class="row">
	<div class="col-sm-3">
		...
		@isset ($wapp)
		<form action="{{ url('admin/web-app',$wapp->id) }}" method="post" class="panel send-form-webapp">
            <div class="panel-heading">Modificazzione utente web app</div>
		@else
		<form action="{{ url('admin/web-app') }}" method="post" class="panel send-form-webapp">
            <div class="panel-heading">Creazione utente web app</div>
		@endisset
			{{ csrf_field() }}
			<div class="panel-body">
				<div class="form-group">
					<label>Cognome Nome</label>
					<input type="text" name="name" class="form-control" value="{{ isset($wapp) ? $wapp->user->name : '' }}">
				</div>

				<div class="form-group">
					<label>Email</label>
					<input type="email" name="email" class="form-control" value="{{ isset($wapp) ? $wapp->user->email : '' }}">
				</div>
				...
				<div class="form-group">
					...
					<input type="text" name="phone" class="form-control" value="{{ isset($wapp) ? $wapp->phone : '' }}">
				</div>

				<div class="form-group">
					<label>Riferimento interno</label>
					<input type="text" name="sin_number" class="form-control" value="{{ isset($wapp) ? ($wapp->user->lastSinister() ? ($wapp->user->lastSinister()->sin_number) : '') : '' }}">
				</div>
				...
				<div class="form-group">
					...
					<select name="society" class="form-control">
						...
						<option {{isset($wapp) ? ($wapp->society == "Renova" ? "selected" : "") : ""}}>Renova</option>
						<option {{isset($wapp) ? ($wapp->society == "Studio Zappa" ? "selected" : "") : ""}}>Studio Zappa</option>
						<option {{isset($wapp) ? ($wapp->society == "Gespea" ? "selected" : "") : ""}}>Gespea</option>
					</select>
				</div>
				...
				<div class="form-group">
					...
					<select name="company" class="form-control">
						<option value="" selected="" disabled=""></option>
						@foreach (App\Company::orderBy('name','asc')->get() as $c)
							<option {{isset($wapp) ? ($c->name == $wapp->company ? 'selected' : '') : ''}} value="{{$c->name}}">{{$c->name}}</option>
						@endforeach
					</select>
				</div>
				...
				<div class="form-group">
					...
					<input size="16" type="text" value="{{isset($wapp) ? ($wapp->expiration ? changeDate($wapp->expiration) : '') : \Carbon\Carbon::now()->addHours(3)->format('d-m-Y H:i')}}" readonly class="form_datetime form-control" name="expiration">
					{{-- <input data-toggle="toggle" data-size="mini" data-on=" " data-off=" " data-onstyle="info" type="checkbox" checked {{ isset($wapp) ? ($wapp->status == 1 ? 'checked' : '') : '' }}> --}}
				</div>
				...
				<div class="tabbable">
					...
					<div class="tab-pane table-responsive active fade in" id="active" style="margin-top: 10px">
						<table class="table table-bordered table-striped orderByDate">
							...
							<tbody>
		                        @foreach (App\WebAppUser::whereExists(function($q){
		                        	$q->from('reservations')
		                        	->whereRaw('reservations.customer_id = web_app_users.user_id')
		                        	->whereRaw('reservations.status = 0');
		                        })->whereExists(function($q){
		                        	/* $q->from('users')
		                        	->whereRaw('users.id = web_app_users.user_id')
		                        	->whereRaw('users.operator_call_id = '.Auth::user()->id);*/
		                        })->where('expiration','>',Carbon\Carbon::now())->get() as $w)
		                            <tr>
		                            	<td style="display: none">{{$w->user->lastSinister()->created_at->format('d-m-Y H:i:s')}}</td>
		                            	<td>WA{{ @str_pad(@$w->id,5,0,STR_PAD_LEFT) }}</td>
		                            	<td>{{ @$w->user->lastSinister()->sin_number}}</td>
		                            	...
		                            	<td>{{ @$w->user->email }}</td>
		                            	<td>
		                            		<span style="display: none">{{strtotime($w->expiration)}}</span>
		                            		{{ changeDate($w->expiration) }}</td>
		                            	<td>
		                            		{{-- <a href="#get-link{{$w->id}}" data-toggle="modal" class="btn btn-xs btn-success"><i class="fa fa-link"></i></a> --}}
		                            		...
		                            		<!-- <div class="modal fade in" id="get-link{{$w->id}}">
		                            			<div class="modal-dialog modal-sm">
		                            				<div class="modal-content">
		                            					...
		                            					<div class="modal-body">
		                            						...
		                            						{{-- <div class="form-group" style="width: 100%">
		                            							<input type="text" class="form-control" placeholder="Telefono per condividere il link" value="+39" style="width: 100%">
		                            						<br>
		                            						<br>
		                            						</div>
		                            						<button class="btn btn-success">Invia SMS</button>
		                            						<a href="{{url('webapp?token='.sha1(@$w->user_id).'&call_id='.@$w->user_id.'&webapp='.$w->id)}}" target="_blank">Copiar link</a> --}}
		                            					</div>
		                            				</div>
		                            			</div>
		                            		</div> -->
