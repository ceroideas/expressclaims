				...
				<div class="panel-body">
					<div class="row">
						<div class="col-sm-12">
							...
							<button type="button" disabled class=" btn btn-sm btn-default" data-target="#sub-{{$s->id}}" data-toggle="modal">{{ $s->claims->count() }} Partite di danno</button>
							<br>
							<br>
						</div>
					</div>
...
<script>
	...
	$('#export-full').click(function(event) {
		$('form.panel').attr('action','{{url('express-tech-export-full')}}');
		$('form.panel').submit();
		$('form.panel').attr('action','{{url('express-tech-export')}}');
	});
