@php
    $claims = App\User::where(function($q) use($r){
        $q->whereExists(function($q) use($r){
            $q->from('customers')
            ...
            $q->whereExists(function($q) use($r){
                ...
                ->whereRaw('reservations.status = 0')->whereExists(function($q) use($r){
                    if (isset($r->search) && $r->search != "") {
                        $q->where('reservations.sin_number','like','%'.$r->search.'%');
                        $q->orWhere('users.name','like','%'.$r->search.'%');
                        $q->orWhere('customers.phone','like','%'.$r->search.'%');
                    }
                });
            });
        });
            ...
            ->whereExists(function($q) use($r){
                ...
                ->whereRaw('reservations.status = 0')->whereExists(function($q) use($r){
                    ...
                    if (isset($r->search) && $r->search != "") {
                        $q->where('reservations.sin_number','like','%'.$r->search.'%');
                        $q->orWhere('users.name','like','%'.$r->search.'%');
                        $q->orWhere('web_app_users.fullphone','like','%'.$r->search.'%');
                    }
                });
            });
        })->where(function($q){
    ...
    // App\User::where('role_id',0)
    // ->where(function($q) use($r){
    //     /*if (isset($r->search)) {
    //         $q->orwhere('name','like','%'.$r->search.'%');
    //     }*/
    // })
    // ->where(function($q){
