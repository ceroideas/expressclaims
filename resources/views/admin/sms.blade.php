...
<div class="row">
	<div class="col-sm-3">
		<form action="{{ url('admin/sms') }}" method="post" class="panel send-form">
			{{ csrf_field() }}
            <div class="panel-heading">Nuovo sms</div>
			<div class="panel-body">
				...
				<button class="btn btn-block btn-success">Invia</button>
			</div>
		</form>
	</div>
		...
		<div class="panel">
			...
			<div class="panel-body">
				<table class="display table table-bordered table-striped">
					...
					<tbody>
                        @foreach (App\SMS::all() as $s)
                            <tr>
                            	<td>{{ $s->id }}</td>
                            	<td>{{ $s->message }}</td>
                            	<td>{{ $s->created_at->format('d-m-Y H:i:s') }}</td>
                            	<td>{{ $s->to }}</td>
                            	<td><a href="{{ url('admin/deleteSms',$s->id) }}" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></a></td>
                            </tr>
                        @endforeach
					</tbody>
				</table>
