...
<div class="row">
	<div class="col-sm-12">
		<form class="panel" method="POST" action="{{ url('express-claims-export') }}" target="_blank">
			<div class="panel-heading">Immagini del sinistro
				...
				{{-- <a href="{{ url('express-claims-export-all',$reservation_id) }}" style="margin-right: 5px;" target="_blank" class="btn btn-info btn-xs pull-right" id="export-all">Esportare tutto</a> --}}
				<button type="button" class="btn btn-info btn-xs pull-right" style="margin-right: 5px;" id="export-all">Esportare tutto</button>
				<a href="{{ url('express-claims-export-zip',$reservation_id) }}" style="margin-right: 5px;" target="_blank" class="btn btn-info btn-xs pull-right">Esportare Zip</a>
			...
			<div class="panel-body">
				<div class="row connectedSortable" id="sortable" data-url="{{url('changeOrderClaims')}}">
	...
	$.each($('.formated_address'), function(index, val) {
		var id = $(this).data('id');
		 ...
		 .done(function(data) {
		 	...
		 	$.post('{{url('updateAddress')}}', {_token: '{{csrf_token()}}',id:id,address:data.results[0].formatted_address}, function(data, textStatus, xhr) {
		 	});
		 })		 
	});
  ...
  $('#check-uncheck').click(function(event) {
  	...
  	$.each($('.formated_address'), function(index, val) {
		var id = $(this).data('id');
		 ...
		 .done(function(data) {
		 	val.innerText = data.results[0].formatted_address;
		 	$.post('{{url('updateAddressClaims')}}', {_token: '{{csrf_token()}}',id:id,address:data.results[0].formatted_address}, function(data, textStatus, xhr) {
		 	});
		 })		 
	});

  	$('#export-full').click(function(event) {
		$('form.panel').attr('action','{{url('express-claims-export-full')}}');
		$('form.panel').submit();
		$('form.panel').attr('action','{{url('express-claims-export')}}');
	});

	$('#export-all').click(function(event) {
		$('form.panel').attr('action','{{url('express-claims-export-all',$reservation_id)}}');
		$('form.panel').submit();
		$('form.panel').attr('action','{{url('express-claims-export')}}');
	});
